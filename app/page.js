  ...
  return (
    <div style={{ height:"100vh", overflow:"auto", background:C.black, position:"relative" }}>
      ...
      <div style={{ position:"fixed", inset:0, pointerEvents:"none", zIndex:0, overflow:"hidden" }}>
        {stars.map(s=>(
          <div key={s.id} style={{ position:"absolute", left:`${s.left}%`, top:`${s.top}%`, width:s.size, height:s.size, borderRadius:"50%", background:"#fff", animation:`twinkle ${s.d}s ${s.delay}s ease-in-out infinite`, opacity:s.op }} />
        ))}
      </div>
      ...
      <section style={{ position:"relative", zIndex:1, minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", textAlign:"center", padding:"120px 24px 80px", overflow:"hidden" }}>
        {[600,900,1200].map((size,i)=>(
          <div key={size} style={{ position:"absolute", width:size, height:size, borderRadius:"50%", border:`1px solid rgba(255,107,43,${0.08-i*0.02})`, top:"50%", left:"50%", animation:`spin ${40+i*25}s linear infinite${i===1?" reverse":""}`, pointerEvents:"none" }} />
        ))}
        ...
        <h1 className="fu1" style={{ fontFamily:"Syne,sans-serif", fontWeight:800, fontSize:"clamp(44px,7vw,82px)", lineHeight:1.0, letterSpacing:"-0.04em", color:"#fff", maxWidth:860, marginBottom:20 }}>
          One dashboard.<br/>
          <span style={{ background:`linear-gradient(90deg,${C.amber},${C.sky})`, WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>All your platforms.</span>
        </h1>

        <p className="fu2" style={{ fontSize:18, color:C.muted, maxWidth:520, lineHeight:1.7, marginBottom:36, fontWeight:300 }}>
          Post to Instagram, X, LinkedIn & more from one place. Easy, fairly priced, with real human support.
        </p>

        {joined ? (
          <div className="fu3" style={{ background:"rgba(255,107,43,0.1)", border:"1px solid rgba(255,107,43,0.3)", borderRadius:12, padding:"14px 28px", color:C.amber, fontWeight:600, fontSize:16 }}>
          ...
          <div className="fu3" style={{ display:"flex", gap:10, justifyContent:"center", flexWrap:"wrap" }}>
            <input value={email} onChange={e=>setEmail(e.target.value)} placeholder="Enter your email address"
              style={{ padding:"13px 18px", borderRadius:10, border:`1px solid ${C.border}`, background:"rgba(255,255,255,0.05)", color:C.text, fontSize:15, outline:"none", fontFamily:"DM Sans,sans-serif", width:290 }} />
            <Btn size="lg" onClick={()=>email.includes("@")&&setJoined(true)} style={{ borderRadius:10 }}>🚀 Join Waitlist</Btn>
          </div>
        )}

        <p className="fu4" style={{ marginTop:14, fontSize:13, color:C.muted }}>
          No credit card · Free forever plan · <span style={{color:C.amber}}>Real human support</span>
        </p>
        ...
        <div className="fu4" style={{ marginTop:52 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:10, flexWrap:"wrap" }}>
            {PLATFORMS.map(p=>(
              <div key={p.id} style={{ display:"flex", alignItems:"center", gap:7, background:C.card, border:`1px solid ${C.border}`, borderRadius:100, padding:"7px 16px", fontSize:13, fontWeight:500, color:C.muted }}>
                <PlatformIcon id={p.id} size={15}/>{p.name}
              </div>
            ))}
          </div>
        </div>
            ...
            : accounts.map(a=>(
              <div key={a.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 0", borderBottom:`1px solid ${C.border}` }}>
                ...
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ fontSize:11, color:C.muted }}>@{a.handle}</div>
                </div>
...
function Compose({ accounts, setPosts, setPage }) {
  ...
  const save = (status) => {
    ...
    setSaved(true)
    setTimeout(()=>setPage("schedule"),1400)
  }
  if (saved) return (
        ...
        <div>
          ...
          <Card style={{ marginBottom:16, overflow:"hidden" }}>
            <textarea value={content} onChange={e=>setContent(e.target.value)} placeholder={"What do you want to share? 🚀\n\nWrite your post here..."}
              style={{ width:"100%", minHeight:180, padding:22, background:"transparent", border:"none", color:C.text, fontSize:15, lineHeight:1.7, fontFamily:"DM Sans,sans-serif" }}/>
            <div style={{ padding:"10px 22px", borderTop:`1px solid ${C.border}`, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div style={{ display:"flex", gap:10 }}>
                {["🖼️","😊","#️⃣"].map(e=><button key={e} style={{background:"none",border:"none",fontSize:17,color:C.muted,cursor:"pointer"}}>{e}</button>)}
              </div>
          ...
          <Card style={{ padding:22, marginBottom:16 }}>
            ...
            <div style={{ display:"flex", gap:8, marginBottom:when==="schedule"?14:0 }}>
              {[["now","⚡ Post now"],["schedule","📅 Schedule"]].map(([v,l])=>(
                <button key={v} onClick={()=>setWhen(v)} style={{ flex:1, padding:"9px", borderRadius:9, border:`1px solid ${when===v?C.orange:C.border}`, background:when===v?"rgba(255,107,43,0.1)":C.surface, color:when===v?C.orange:C.muted, fontSize:13, fontWeight:500, cursor:"pointer" }}>{l}</button>
              ))}
            </div>
          ...
          <Card style={{ padding:22, position:"sticky", top:0 }}>
            ...
            <div style={{ background:C.surface, borderRadius:12, padding:16 }}>
              <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:12 }}>
                ...
                <div>
                  <div style={{ fontSize:13, fontWeight:600, color:"#fff" }}>Your Name</div>
                  <div style={{ fontSize:11, color:C.muted }}>@yourhandle</div>
                </div>
              </div>
            ...
            <div style={{ marginTop:16, borderTop:`1px solid ${C.border}`, paddingTop:14 }}>
              ...
              {selected.length===0?<div style={{fontSize:13,color:C.muted}}>None selected</div>:selected.map(p=>(
                <div key={p} style={{ display:"flex", alignItems:"center", gap:8, marginBottom:8 }}>
                  <PlatformIcon id={p} size={14}/>
                  <span style={{ fontSize:13, color:C.text }}>{PLATFORMS.find(pl=>pl.id===p)?.name}</span>
                </div>
              ))}
            </div>
          </Card>
  ...
  return (
    <div style={{ padding:"32px 36px", overflowY:"auto", height:"100%" }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:24 }}>
        <div>
          <h1 style={{ fontFamily:"Syne,sans-serif", fontSize:26, fontWeight:800, color:"#fff", marginBottom:4 }}>Schedule 📅</h1>
          <p style={{ color:C.muted, fontSize:14 }}>{posts.length} total posts</p>
        </div>
        <Btn onClick={()=>setPage("compose")}>+ New Post</Btn>
      </div>
      <div style={{ display:"flex", gap:8, marginBottom:20 }}>
        {[["all","All"],["scheduled","📅 Scheduled"],["published","✅ Published"],["draft","📝 Drafts"]].map(([v,l])=>(
          <button key={v} onClick={()=>setFilter(v)} style={{ padding:"7px 16px", borderRadius:100, fontSize:13, fontWeight:500, border:`1px solid ${filter===v?C.orange:C.border}`, background:filter===v?"rgba(255,107,43,0.1)":"transparent", color:filter===v?C.orange:C.muted, cursor:"pointer" }}>
            {l} {v!=="all"&&`(${posts.filter(p=>p.status===v).length})`}
          </button>
        ))}
      </div>
      {filtered.length===0
  ...
  return (
    <div style={{ padding:"32px 36px", overflowY:"auto", height:"100%" }}>
          ...
          {accounts.map(a=>(
            <div key={a.id} style={{ display:"flex", alignItems:"center", gap:14, padding:"14px 0", borderBottom:`1px solid ${C.border}` }}>
              ...
              <div style={{ flex:1 }}>
                ...
                <div style={{ fontSize:12, color:C.muted }}>@{a.handle} · {a.platform}</div>
              </div>
      ...
      <Card style={{ padding:24 }}>
        ...
        <p style={{ fontSize:13, color:C.muted, marginBottom:22 }}>Click any platform to connect it</p>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:10 }}>
          {PLATFORMS.map(p=>{
            const isConn=connected.includes(p.id); const isAdd=adding===p.id
            return (
              <div key={p.id}>
                <button onClick={()=>!isConn&&setAdding(isAdd?null:p.id)} style={{ width:"100%", padding:"16px 10px", borderRadius:13, border:`1px solid ${isAdd?C.orange:isConn?"rgba(74,222,128,0.3)":C.border}`, background:isAdd?"rgba(255,107,43,0.08)":isConn?"rgba(74,222,128,0.05)":C.surface, display:"flex", flexDirection:"column", alignItems:"center", gap:8, cursor:isConn?"default":"pointer", transition:"all 0.2s" }}>
                  <PlatformIcon id={p.id} size={26}/>
                  <span style={{ fontSize:12, fontWeight:500, color:isConn?C.green:C.text }}>{p.name}</span>
                  <span style={{ fontSize:10, color:isConn?C.green:C.muted }}>{isConn?"✓ Connected":isAdd?"Cancel":"+ Add"}</span>
                </button>
                {isAdd&&(
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:14, marginBottom:24 }}>
        {[{label:"Published",val:published.length,icon:"📝"},{label:"Platforms",val:accounts.length,icon:"🔗"},{label:"Drafts",val:posts.filter(p=>p.status==="draft").length,icon:"📋"},{label:"Scheduled",val:posts.filter(p=>p.status==="scheduled").length,icon:"📅"}].map(s=>(
          <Card key={s.label} style={{ padding:22 }}>
            ...
            <div style={{ fontFamily:"Syne,sans-serif", fontSize:32, fontWeight:800, color:"#fff", marginBottom:2 }}>{s.val}</div>
            <div style={{ fontSize:12, color:C.muted }}>{s.label}</div>
          </Card>
        ))}
      </div>
...
function Dashboard({ user, onLogout }) {
  ...
  const pages = { overview:<Overview posts={posts} accounts={accounts} setPage={setPage}/>, compose:<Compose accounts={accounts} setPosts={setPosts} setPage={setPage}/>, schedule:<Schedule posts={posts} setPosts={setPosts} setPage={setPage}/>, accounts:<Accounts accounts={accounts} setAccounts={setAccounts}/>, analytics:<Analytics posts={posts} accounts={accounts}/> }
  return (
